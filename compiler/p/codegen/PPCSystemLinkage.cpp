...
void TR::PPCSystemLinkage::createPrologue(TR::Instruction *cursor, List<TR::ParameterSymbol> &parmList)
{
    ...
    if (machine->getLinkRegisterKilled())
        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
            TR::MemoryReference::createWithDisplacement(cg(), sp, 2 * TR::Compiler->om.sizeofReferenceAddress(),
    ...
    if (savedFirst <= TR::RealRegister::LastGPR) {
        if (cg()->comp()->target().cpu.is(OMR_PROCESSOR_PPC_GP) || cg()->comp()->target().is64Bit()
            ...
            for (regIndex = TR::RealRegister::LastGPR; regIndex >= savedFirst;
                ...
                cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
                    TR::MemoryReference::createWithDisplacement(cg(), sp, argSize,
                        TR::Compiler->om.sizeofReferenceAddress()),
    ...
    if (bodySymbol->isEHAware() || machine->getRealRegister(TR::RealRegister::cr2)->getHasBeenAssignedInMethod()
        ...
        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
            TR::MemoryReference::createWithDisplacement(cg(), sp, TR::Compiler->om.sizeofReferenceAddress(),
                TR::Compiler->om.sizeofReferenceAddress()),
            gr0, cursor);
    }
        ...
        if (cg()->comp()->target().cpu.is(OMR_PROCESSOR_PPC_GP) || cg()->comp()->target().is64Bit()
            ...
            for (regIndex = TR::RealRegister::LastGPR; regIndex >= savedFirst;
                ...
                cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, currentNode,
                    ...
                    TR::MemoryReference::createWithDisplacement(cg(), sp, saveSize,
                        TR::Compiler->om.sizeofReferenceAddress()),
                    cursor);
            }
        else {
            ...
            default:
                ...
                if (numVectorArgs < properties.getNumVectorArgRegs()) {
                    if (!cg()->canClobberNodesRegister(child, 0)) {
                        ...
                        generateTrg1Src2Instruction(cg(), TR::InstOpCode::xxlor, callNode, tempRegister, argReg,
                            argReg);
                        argReg = tempRegister;
                    }
                    if (numVectorArgs == 0) {
                        ...
                        TR::addDependency(dependencies, argReg, properties.getVectorArgumentRegister(numVectorArgs),
                            TR_VRF, cg());
                } else {
...
void TR::PPCSystemLinkage::buildVirtualDispatch(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
    uint32_t sizeOfArguments)
{
    ...
    if (cg()->comp()->target().cpu.isBigEndian()) {
        ...
        generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, gr0,
            TR::MemoryReference::createWithDisplacement(cg(), callNode->getChild(0)->getRegister(), 0,
                TR::Compiler->om.sizeofReferenceAddress()));
