...
TR::PPCSystemLinkage::createPrologue(
   ...
   if (size > self()->getOffsetToFirstParm() - argSize)
      ...
      if (size > (-LOWER_IMMED))
         ...
         {
         //NOTE (remove this comment later): the fact that the displacement here is negative might mess things up when displacement is changed from 32 bits to 64 bits
