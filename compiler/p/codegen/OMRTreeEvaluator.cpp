...
TR::Register *OMR::Power::TreeEvaluator::vstoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P9) && cg->comp()->target().cpu.isLittleEndian()
        ...
        if (et == TR::Int8) {
            generateTrg1ImmInstruction(cg, TR::InstOpCode::vspltish, node, rotateReg, 8);
            generateTrg1Src2Instruction(cg, TR::InstOpCode::vrlh, node, rearrangedValueReg, rearrangedValueReg, rotateReg);
        }

        cg->stopUsingRegister(rotateReg);
    }

    TR::LoadStoreHandler::generateStoreNodeSequence(cg, rearrangedValueReg ? rearrangedValueReg : valueReg, node, opcode, 16, true);
